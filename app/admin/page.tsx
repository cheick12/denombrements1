...
export const metadata: Metadata = {
  title: 'Administration — Soumaoro Signature',
...
> = {
  ...
  in_progress: { label: 'En cours', color: 'text-gold bg-gold/10 border-gold/20', icon: Car },
  completed: { label: 'Terminée', color: 'text-green-400 bg-green-400/10 border-green-400/20', icon: CheckCircle },
...
export default async function AdminPage() {
  ...
  const statCards = [
    { icon: Calendar, label: 'Réservations totales', value: stats.totalBookings, sub: `${stats.bookingsThisMonth} ce mois` },
    { icon: CheckCircle, label: 'Confirmées', value: stats.confirmedBookings, sub: 'Hors annulées' },
    { icon: Euro, label: 'Revenus totaux', value: formatPrice(stats.totalRevenue), sub: `${formatPrice(stats.revenueThisMonth)} ce mois` },
    { icon: MessageSquare, label: 'Messages', value: messages.filter((m) => !m.read).length, sub: `${messages.length} au total` },
  ]
      ...
      <div className="bg-dark-100 border-b border-white/5 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <span className="text-white font-serif font-semibold">
            Tableau de bord — Soumaoro Signature
          </span>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          <div className="lg:col-span-2">
            <div className="glass-card rounded-xl overflow-hidden">
              <div className="flex items-center justify-between px-6 py-4 border-b border-white/5">
                ...
                <span className="text-white/30 text-xs">{bookings.length} au total</span>
              </div>
              ...
              {bookings.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <p className="text-white/20 text-xs mt-1">
                    Les réservations apparaîtront ici dès réception
                  </p>
                </div>
              ) : (
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-white/5">
                        {['ID', 'Client', 'Trajet', 'Date', 'Prix', 'Statut'].map((h) => (
                          <th
                            key={h}
          ...
          <div>
            <div className="glass-card rounded-xl overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-white/5">
                <h2 className="text-white font-semibold">Messages récents</h2>
                <span className="text-white/30 text-xs">{messages.length} au total</span>
              </div>
        ...
        <div className="mt-6 glass-card rounded-xl overflow-hidden">
          <div className="px-6 py-4 border-b border-white/5">
            <h2 className="text-white font-semibold">Prochaines courses</h2>
          </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="text-white/40 text-xs">
                          {booking.departure} → {booking.arrival || 'Mise à disposition'}
                        </div>
                      </div>
                    </div>
